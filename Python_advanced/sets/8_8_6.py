...
set_a = {sentence[i].strip('();:.,').lower() for i in range(len(sentence)) if len(sentence[i].strip('();:.,')) < 4}

print(*sorted(set_a))

# The program outputs a set containing the unique words
# (in lower case) of the sentence string less than 4 characters long.
